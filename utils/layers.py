...
# an easy way to support batches > 1 quick 从ly with little code modification.
...
class SingleHeadAttention(KL.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        # 应用注意力权重
        output = tf.matmul(attention_weights, v)  # (batch_size, seq_len_q, d_model)

        # 最终投影
        output = K.dot(output, self.dense_w) + self.dense_bias  # (batch_size, seq_len_q, d_model)
...
def encoder_graph_single_head(d_model, dff, training, rate, x):
    """使用单头注意力的编码器层"""
    attn_output = SingleHeadAttention(d_model)([x, x, x])  # (batch_size, input_seq_len, d_model)
    attn_output = KL.Dropout(rate=rate)(attn_output, training=training)
    out1 = tf_layers.LayerNormalization(epsilon=1e-6)(KL.Add()([x, attn_output]))

    ffn_output = feed_forword_graph(d_model, dff, out1)
    ffn_output = KL.Dropout(rate=rate)(ffn_output, training=training)
    out2 = tf_layers.LayerNormalization(epsilon=1e-6)(KL.Add()([out1, ffn_output]))
